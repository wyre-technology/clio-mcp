 ...
 *
 * Domain handlers don't otherwise have access to the server instance, so
 * they call `getServerRef()` to reach `server.elicitInput()` (skill 2.8).
 *
 ...
 * inside async tool handlers (e.g. after awaiting a Clio API call, before
 ...
 * request (skill 2.3), so two concurrent requests race through that shared
 * global: tenant A's request sets the ref and starts awaiting async work;
 * before A resumes, tenant B's request runs and overwrites the module-level
 * ref with B's server/transport; when A's awaited work resolves and it
 * reads the ref back to call `elicitInput`, it gets B's server and A's
 * confirmation prompt is sent down B's connection instead of A's.
 *
